...
`),
	FlagSet: (func() *flag.FlagSet {
		...
		upf.StringVar(&upArgs.defaultVia, "exit-node", "", "Tailscale IP of the machine through which to route all traffic")
...
func runUp(ctx context.Context, args []string) error {
	...
	var defaultVia netaddr.IP
	if upArgs.defaultVia != "" {
		var err error
		defaultVia, err = netaddr.ParseIP(upArgs.defaultVia)
		if err != nil {
			fatalf("invalid IP address %q for --exit-node: %v", upArgs.defaultVia, err)
		}
	}
	...
	prefs.DefaultRouteVia = defaultVia
